...
class FenixBrowser:
    ...
    def ensure_logged_in(self) -> None:
        ...
        print(
            "[INFO] Checking the saved Fenix login..."
        )
        ...
        # Existing saved session is still valid.
        if "/login" not in self.page.url.lower():
            print(
                "[INFO] Existing Fenix session is valid."
            )
        ...
        print(
            "[INFO] Saved Fenix session expired. "
        ...
        if not username or not password:
            raise RuntimeError(
                ...
                "Add FENIX_USERNAME and FENIX_PASSWORD and restart the app."
            )
        ...
        if username_field is None:
            raise RuntimeError(
                "Automatic Fenix login could not find "
                "the username field."
            )
        ...
        print(
            "[INFO] Entering saved Fenix credentials..."
        )
        ...
        if "/login" in self.page.url.lower():
            raise RuntimeError(
                "Automatic Fenix login failed. "
                "The username/password may be incorrect, "
                "or Fenix may require an additional login step."
            )

        print(
            "[SUCCESS] Fenix automatic login completed."
        )

        # Explicitly return to Search Stock after login.
        self.page.goto(
            PORTAL_URL,
            wait_until="domcontentloaded",
            timeout=90_000,
        ...
        if "/login" in self.page.url.lower():
            raise RuntimeError(
                "Fenix logged in but redirected "
        ...
        print(
            "[SUCCESS] Fenix session refreshed and saved."
        )
